...
export default function Landing() {
  ...
  const [activeStep, setActiveStep] = useState(0); // 0: section1, 1: card1, 2: card2, 3: card3, 4: section3
  const totalCards = 3;
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      setActiveStep((prev) => {
        ...
        if (next < 0) next = 0;
        if (next > totalCards + 1) next = totalCards + 1; // 3 کارت + section3
        return next;
      });
    };
  ...
  useEffect(() => {
    const container = containerRef.current;
    // بر اساس activeStep، اسکرول می‌کنیم
    if (activeStep === 0) {
